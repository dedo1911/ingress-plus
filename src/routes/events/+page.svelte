<script>
  ...
  const loadData = async () => {
    ...
    const events = (
      await pb.collection("game_events").getFullList({
        sort: "start_time",
        order: "desc",
      ...
      e = {
        ...
        repeat_cron: e.repeat_cron
          ? dayjs(
              cronjsMatcher.getFutureMatches(e.repeat_cron, {
      ...
      if (e.repeat_cron) {
        e.start_time = e.start_time
          .year(e.repeat_cron.year())
          .month(e.repeat_cron.month())
          .date(e.repeat_cron.date());
    ...
    activeEvents = events.filter(
      (e) => e.start_time.isBefore(dayjs()) && e.end_time.isAfter(dayjs())
    );
    futureEvents = sortBy(
      events.filter((e) => e.start_time.isAfter(dayjs())),
      "start_time"
    );
    pastEvents = sortBy(
      events.filter((e) => e.end_time.isBefore(dayjs())),
      "start_time"
    ).reverse();
...
<div class="container">
  {#if activeEvents.length > 0}
    ...
    <div transition:slide>
      ...
      <Pagination
        rows={activeEvents}
        perPage={5}
        bind:trimmedRows={activeEventsPage}
      />
    </div>
    <hr />
  {/if}
  ...
  {#if activeSection === "future"}
    <div transition:slide>
      ...
      <Pagination
        rows={futureEvents}
        perPage={5}
        bind:trimmedRows={futureEventsPage}
      />
    </div>
  {/if}
  ...
  {#if activeSection === "past"}
    <div transition:slide>
      ...
      <Pagination
        rows={pastEvents}
        perPage={10}
        bind:trimmedRows={pastEventsPage}
      />
    </div>
  {/if}
  ...
  <div class="options">
    <button onclick={toggleShowAll} title={showAll ? "Show less" : "Show more"}>
      <img
        src="/images/{showAll ? 'checkbox_on' : 'checkbox_off'}.png"
        alt="Checkbox"
      /> Show All Events</button
    >
    <p>
      <small
        >(including Events that might not be relevant to you like Mission Days)</small
      >
    </p>
  </div>
</div>
